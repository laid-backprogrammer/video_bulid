...
export function makeCodeWriterSystemPrompt() {
  return [
    ...
    'scene.tuningNotes are the highest-priority current user overrides. They supersede conflicting older scene.designNotes.',
    'scene.designNotes are the base creative brief. Use only the parts that do not conflict with scene.tuningNotes.',
    'Turn the effective brief into bespoke Remotion visuals instead of adapting a generic title/caption template.',
    ...
    'If user media assets are provided, use every required asset explicitly @mentioned and listed in scene.assets. Do not use other runtime assets even if they exist in the props.',
    ...
    'Never copy public/assets/scenes paths into generated code, even if designNotes contains them. Select media from the runtime assets prop by explicit mentioned id or alias first, then verify role/assetType, and render nothing or a fallback when no matching asset exists.',
    'Do not select media only by assetType or only by role, for example assets.find((asset) => asset.assetType === "image"). Every rendered uploaded media asset must have an id- or alias-specific selector such as asset.id === "asset_..." or asset.alias === "hero".',
    ...
    'Every interpolate() inputRange must be strictly increasing at runtime. Be careful when mixing word frames, hard-coded beat frames, and durationInFrames because generated scenes may preview with shorter or stale manifest durations.',
    'If an inputRange depends on durationInFrames, cue timings, or word timings, use a small helper that normalizes points to strictly increasing values before calling Remotion interpolate(). Never pass a dynamic unsanitized input array directly into interpolate().',
...
export function makeVisualDirectorSystemPrompt() {
  return [
    ...
    'Keep the whole answer under 900 tokens. Use short bullets only.',
    'Return exactly these sections: Visual, Timeline, Captions, Assets, Avoid.',
    'Preserve only hard requirements: user tuning, required assets, pacing, subtitle placement, colors, and named visual objects.',
    'If tuningNotes conflict with designNotes, follow tuningNotes without long explanation.',
    'For each required media asset, include id/alias and the exact selector rule.',
...
export function makeCodeWriterUserPrompt(contextMarkdown, blueprint) {
  return [
    ...
    'For every required asset, include an id- or alias-specific selector in code. Do not rely on generic assets.find/filter by assetType or role.',
...
export function makeRepairUserPrompt({repairContextMarkdown, code}) {
  return [
    ...
    'Make the minimum necessary contract repair. Do not redesign the visual style, layout, colors, or animation logic unless the validation error specifically says the layout is generic or weakly aligned to the brief.',
    '',
...
export function buildFallbackBlueprint(context) {
  ...
  return [
    ...
    [
      'Highest priority tuning/user overrides:',
      scene.tuningNotes || 'No fine-tuning notes provided.',
      '',
      'Base design brief, only where not superseded by tuning:',
    ...
    assets.length
      ? [
        ...
        'Select every rendered uploaded asset by explicit id or alias first. Do not select assets only by assetType or role.',
      ].join('\n')
      : 'No @mentioned user media assets or visual references were provided.',
    '',
    '# Required Asset Plan',
    assets.length
      ? assets.map((asset) => `- @${asset.alias || asset.id}: ${asset.assetType || 'image'} / ${asset.role || 'both'} / ${asset.notes || 'no placement notes'} / selector: asset.id === "${asset.id}"${asset.alias ? ` or asset.alias === "${asset.alias}"` : ''}`).join('\n')
      : 'No required render assets.',
    '',
